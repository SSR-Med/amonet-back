...
class DeleteTagKanbanCommandHandler:
    ...
    async def handle(
        self, command: DeleteTagKanbanCommand, current_user: CurrentUserDto
    ) -> None:
        model = await self._repository.first_or_default(
            lambda q: q.where(TagKanbanConfiguration.id_amonet_tag_kanban == command.id)
        )
        if model is None:
            raise NotFoundException("TagKanban", str(command.id))
