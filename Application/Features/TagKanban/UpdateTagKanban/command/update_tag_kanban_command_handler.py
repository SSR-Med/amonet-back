...
from Application.Features.TagKanban.UpdateTagKanban.mappers import (
    UpdateTagKanbanMapper,
)
...
class UpdateTagKanbanCommandHandler:
    ...
    async def handle(
        self, id: UUID, command: UpdateTagKanbanCommand, current_user: CurrentUserDto
    ) -> TagKanbanResponseDto:
        ...
        existing_color = await self._repository.first_or_default(
            lambda q: q.where(
                ...
                TagKanbanConfiguration.id_amonet_tag_kanban != id,
            )
        )
        ...
        model = await self._repository.first_or_default(
            lambda q: q.where(TagKanbanConfiguration.id_amonet_tag_kanban == id)
        )
        if model is None:
            raise NotFoundException("TagKanban", str(id))

        existing = await self._repository.first_or_default(
            lambda q: q.where(
                TagKanbanConfiguration.nombre == command.nombre,
                TagKanbanConfiguration.id_amonet_tag_kanban != id,
            )
        )
        ...
        model = UpdateTagKanbanMapper.apply(model, command)
